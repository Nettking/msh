...
When a web-capable mode is selected, setup can also enable and install a local Ollama model for `/ai`. The three standard choices are:
...
- `MSH_AI_MODEL` — selected local Ollama model.
...
- <http://localhost:5000/ai> — ask read-only system-understanding questions using the selected local model.
