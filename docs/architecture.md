...
## Federated session network proposal

The current architecture is local and single-process. A proposed future architecture allows nodes on different physical networks to join a shared, versioned session and contribute multiple instances of storage, recording, language-model, compute, and other capabilities. It separates a small coordination control plane from a user-contributed data plane, uses outbound node connections with relay fallback, and defines completeness-aware storage leadership with replicas, leases, terms, and fencing tokens.
