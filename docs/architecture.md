...
Each `catalog/<script>/` directory contains a runnable analysis script and usually a script-specific README. The top-level `catalog/README.md` explains script categories and workflow order.
...
- **Playback coverage:** automatic startup/catch-up is bounded to health checks plus timeline/playback generation.
...
The artifact catalog scans configured roots such as `results` and `data`. Artifacts can be generated by automatic scripts, manual scripts, playback export helpers, or external preparation. Internal runtime files may be surfaced separately from default user-facing analysis artifacts.
...
- Manual/deep analysis scripts may be slow and are excluded from default bootstrap.
