...
Status: automated product acceptance and the physical evidence contract are merged on `main`. **Physical acceptance has not been performed or claimed.**
...
A green CI workflow proves only the first category and the correctness of the physical evidence validator. Full capability-first and Federation v1 end-to-end acceptance remain false until the physical document validates against the exact accepted candidate.
...
- a fresh state root with no Federation candidate;
...
Copy the template to `evidence/cf7-physical-evidence.json`. Replace placeholders only after the corresponding observation has passed.
...
`git status --short` must be empty before the physical run.
...
CF7 may be declared complete only when all conditions are true for the same exact commit:
