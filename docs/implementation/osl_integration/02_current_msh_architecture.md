# Current MSH architecture relevant to OSL

Status: verified architecture inventory at MSH commit
...
`existing-in-MSH` has a useful statement-first operator-note workflow, related
...
- `existing-in-MSH`: `docs/architecture.md` lines 1--3 describe MSH as a
  Flask-first orchestration and analysis system for CNC telemetry.
- `existing-in-MSH`: `catalog/flask_app/` is the main UI/application layer;
  ...
  lines 54--112).
- `existing-in-MSH`: raw telemetry JSONL remains source of truth while Parquet
...
- `existing-in-MSH`: operator knowledge intentionally uses JSON rather than
...
- `proposed-for-MSH`: place the canonical OSL domain/application layer outside
...
| --- | --- | --- | --- |
| Knowledge capture | `existing-in-MSH` `OperatorStrategyService.add_from_form` stores one fast raw statement and optional fields. | Capture UX, timestamps, source inventory lookup, non-JSONL location. | `proposed-for-MSH` immutable source artefact/session and excerpt layer; never auto-map raw text to Decision. |
| Record structuring | `existing-in-MSH` flat `OperatorStrategyRecord` with paper-like strings. | Legacy import vocabulary and human-facing prompts. | Typed nodes/relations, immutable revisions, explicit missing fields, source trace. |
| Lifecycle | `existing-in-MSH` `captured`, `structured`, `reusable` plus boolean `reusable_strategy`. | Wording only as a legacy migration hint. | Separate extraction, draft, review, approval, publication, supersession, and OSL maturity. |
| Storage | `existing-in-MSH` whole JSON-file read/modify/temp-replace. | Atomic file replacement for a small legacy store. | Transactional repository, optimistic revision, command idempotency, append-only history. |
| Provenance | `existing-in-MSH` IDs/timestamps and free-text evidence/trace fields. | Original legacy record ID as import provenance. | Source/excerpt hashes, actors, transformations, reviews, revisions, validation and export lineage. |
| AI | `existing-in-MSH` read-only repository explainer and bounded local/remote provider runtime. | Provider/runtime contracts, session fencing, attempts/results, safe presentation. | New candidate-only application service with data classification, structured validation, and no lifecycle authority. |
| Authorization | `existing-in-MSH` strong authority mechanisms in capability/federation subsystems, but none on operator-note routes. | Fail-closed server-bound actor/scope patterns. | Human OSL policy/authentication and CSRF; never reuse storage/compute authority as strategy approval. |
| Flask/API | `existing-in-MSH` server-rendered capture/review/structure POST routes. | Blueprint, thin-route, service and test-client conventions. | Authenticated command/query routes with expected revision and server-derived actor/scope. |
| UI | `existing-in-MSH` statement-first capture and raw-text display. | Navigation, responsive patterns, empty/error conventions. | Side-by-side source/derivation, provenance, validation, version comparison, review/approval, accessibility. |
| Federation | `existing-in-MSH` authenticated, revisioned storage/session protocols and public projections. | Persistence, redaction and degraded-state design patterns. | Keep initial OSL local; federation is an optional later repository adapter/projection with separate policy. |
| Capabilities | `existing-in-MSH` versioned jobs, AI/compute providers, idempotent state machines, artifact grants. | Contract and transaction patterns. | OSL-specific types and authority; AI capability cannot gain storage, approval, or execution rights. |
| Events/audit | `existing-in-MSH` append audit in capability/federation stores. | Ordered, actor/command-bound events. | Unbounded/retention-governed OSL provenance distinct from operational audit. |
| Import/export | `existing-in-MSH` one-way heuristic SysML and recommender JSON generation; no OSL import. | Compatibility facade and legacy test fixtures. | Canonical codecs, deterministic faithful adapters, validation, history, collision handling, import quarantine. |
| Tests/CI | `existing-in-MSH` pytest/Ruff conventions and multiple Ubuntu/Windows workflow matrices. | Cross-platform gate structure and focused tests. | Permanent OSL gate; current operator/OSL tests are not selected by any workflow. |

## Knowledge capture, observations, and records

### Existing behavior

`existing-in-MSH` `catalog/flask_app/services/operator_strategy_service.py`:

- lines 19--21 define
  `data/operator_strategy_records/operator_strategies.json` and schema
  `msh.operator_strategy_records.v3`;
...
- `existing-in-MSH` `operator_capture.html` lines 13--42 requests one raw
  statement first.
- `existing-in-MSH` `operator_structure.html` lines 10--16 shows the original
  while structuring.
- `existing-in-MSH` `docs/operator_strategy_capture.md` lines 23--45 and 61--92
...
- `existing-in-MSH` design intent in
  `docs/msh_operator_support_plan.md` lines 23--59 and 119--127 explicitly says
  to keep raw statements separate from interpretation.

### Semantic and durability gaps

- `existing-in-MSH` contradiction: `add_from_form` lines 136--137 copies
...
- `existing-in-MSH` `operator_structure.html` line 39 combines “Decision /
  strategy action,” so `Decision` and `OperatorAction` have neither separate
  identity nor a typed relationship.
- `existing-in-MSH` `evidence`, `trace_target`, `confidence`,
...
- `existing-in-MSH` `update_structure` and `update_outcome` overwrite prior
  values; there is no author, base revision, change reason, or previous version.
- `existing-in-MSH` `mark_reusable` lines 222--231 can move any record directly
...
- `existing-in-MSH` `delete` erases the record and its source text. This is
  incompatible with immutable evidence and durable provenance.
- `existing-in-MSH` whole-file read/modify/write has no lock, expected revision,
  or idempotency command and can lose concurrent changes.
- `existing-in-MSH` unknown per-record schema/status values are normalized
  permissively rather than rejected, quarantined, or migrated explicitly.

`proposed-for-MSH`: retain this file as a read-only legacy source during
...
- `existing-in-MSH` `MachineNotesService` stores machine-scoped text,
...
- `existing-in-MSH` `operator_support_model.py` lines 19--130 defines frozen
...
- `existing-in-MSH` `OperatorSupportService` lines 49--95 combines seed rules
  and reusable notes into recommendation cards. It synthesizes text such as a
  count of related notes as “evidence.”
- `existing-in-MSH` `OperatorConfirmationService` lines 22--64 accepts
...
- `existing-in-MSH` first-part, quality-outcome, and confirmation records are
  separate loose JSON stores and use user-entered links/signers.
- `existing-in-MSH` `StrategyComparisonService` lines 13--58 groups notes by
...
- `existing-in-MSH` raw telemetry JSONL and MTConnect observations have their own
  source, timestamp, normalization, gap, and playback contracts
  (`docs/data_contract.md` lines 5--108 and 147--157).
- `existing-in-MSH` intervention strategies are YAML-configured heuristics that
...
- `existing-in-MSH` the documentation explicitly says these candidates are not
...
`existing-in-MSH` is a local JSON document written through a temporary file and
...
- `existing-in-MSH` `catalog/capabilities/contributions/store.py` lines 91--159
  uses a transaction and deterministic revision history for local intent.
- `existing-in-MSH` `catalog/federation/local_storage.py` lines 1--4 and
  172--278 demonstrates immutable ingest, content hashes, atomic publication,
  and durable idempotency.
- `existing-in-MSH` federation logical storage requires separate session,
  provider, group, and write-authority context.

`proposed-for-MSH`: build a dedicated local transactional OSL repository using
...
- `existing-in-MSH` operator notes have a UUID, capture/decision timestamps,
...
- `existing-in-MSH` the AI explainer is read-only and grounded in selected
  ...
  42--52, and 92--106).
- `existing-in-MSH` `catalog/ai/prompts.py` lines 1--13 reinforces that the
  model explains rather than mutates.
- `existing-in-MSH` `catalog/ai/repo_index.py` excludes data/results paths, so
  the current explainer cannot process operator captures.
- `existing-in-MSH` `catalog/ai/runtime_contracts.py` defines bounded,
...
- `existing-in-MSH` connected provider documentation says a language-model
...
`proposed-for-MSH` reuse the runtime/provider ports for inference, not the
...
operator knowledge. `proposed-for-MSH` therefore requires source-level
...
- `existing-in-MSH` `SQLiteArtifactAuthority` is fail-closed and binds grants,
  scopes, placement, expiry, hashes, revocation, and audit
  (`catalog/capabilities/artifact_authority.py` lines 53--165 and 321--1017).
- `existing-in-MSH` federation write authority uses session/group/actor,
  current grants, terms, fencing tokens, and lease expiry
  (`catalog/federation/write_authority.py` lines 1--110).
- `existing-in-MSH` capability/job ownership and provider selection use explicit
  transitions and server-side state rather than UI assertions.

`proposed-for-MSH` borrow fail-closed actor/scope/fencing principles, but define
...
- `existing-in-MSH` Flask blueprints and service-oriented routes are registered
  centrally in `catalog/flask_app/app.py` lines 107--119.
- `existing-in-MSH` statement-first capture is fast and mobile-oriented.
- `existing-in-MSH` review pages already expose empty/load-error states and
  preserve visible raw text.
- `existing-in-MSH` base templates include responsive viewport/navigation, and
  the test suite contains responsive/mobile/static CSS contract tests.
- `existing-in-MSH` source inventory can suggest machine/sensor references.

### What must change later

- `existing-in-MSH` `operator_review.html` lines 38--46 can mark reusable or
  delete immediately; `operator_structure.html` lines 16--60 posts a combined
  edit/reusable form.
- `existing-in-MSH` routes instantiate local file services directly and pass raw
  records to templates rather than authorization-filtered projections.
- `existing-in-MSH` `_source_inventory` catches every exception and returns an
  empty inventory (`operator_strategy_routes.py` lines 16--20), masking cause
  and freshness.
- `proposed-for-MSH` keep routes thin: commands carry CSRF, idempotency key, and
  expected revision; actor/scope comes from authenticated server context;
  queries return explicit safe projections.
- `proposed-for-MSH` UI is never source of truth. It must display source versus
...
`existing-in-MSH` federation and capabilities are relevant as architectural
...
`existing-in-MSH` `catalog/flask_app/services/osl_export_service.py`:
...
`existing-in-MSH` `catalog/flask_app/test_osl_sysml_export.py` lines 13--60
...
- `existing-in-MSH` `RecommenderArtifactService` lines 18--40 overwrites derived
  JSON from reusable notes and invents fallback evidence/reason text.
- `existing-in-MSH` there is no OSL import path.
...
`existing-in-MSH` `msh.operator_strategy_records.v3` identifies only the outer
...
- `SCHEMA` constants such as `msh.node.v1`, `msh.session.v1`, and
  `msh.job.v1`;
...
`proposed-for-MSH` apply those conventions in a new OSL package, with language
...
- `existing-in-MSH` tests commonly use `tmp_path`, `monkeypatch`, Flask
  `test_client`, frozen-contract round trips, malformed/size/idempotency/restart
  cases, and targeted concurrency/race tests.
- `existing-in-MSH` `test_operator_strategy_service.py` verifies fast capture
...
- `existing-in-MSH` `test_operator_strategy_lifecycle.py` permits
  outcome/reusable transitions without semantic review.
- `existing-in-MSH` `test_osl_sysml_export.py` is string-only.
- `existing-in-MSH` route/template tests cover capture text, review wording,
...
- `existing-in-MSH` no workflow path filter or test command selects the current
  operator-strategy/OSL files. They therefore lack a permanent merge gate.
- `existing-in-MSH` `docs/ci-lint-baseline.md` lines 3--7 treats
  workflow-specific Ruff exclusions as temporary, not precedent for new debt.

`proposed-for-MSH` later add one permanent cross-platform OSL workflow covering
...
1. `proposed-for-MSH` introduce a new `catalog/osl/` domain/application boundary;
   do not grow `OperatorStrategyRecord` into the canonical model.
2. `proposed-for-MSH` use a local transactional repository first; add federation
   only through a later authorized adapter.
3. `proposed-for-MSH` preserve the legacy store untouched during idempotent
   migration and map ambiguity to findings.
4. `proposed-for-MSH` require authentication/authorization and CSRF before
   exposing sensitive source, review, approval, or publication commands.
5. `proposed-for-MSH` make operator-support, recommender, comparison, UI, API,
...
6. `requires-research-clarification` decide whether federation-visible
   publication is needed at all; current MSH federation has mechanisms but no
