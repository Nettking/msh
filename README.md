...
Webapp-first repository for recording, scanning, and inspecting MTConnect telemetry analyses through a continuous digital twin interface.
...
1. **Streamlit web app (`webapp`)** as the main always-on digital twin workspace.
2. **CLI runner (`msh`)** as a secondary/manual workflow for script execution.

### Primary workflow: Streamlit web app (`webapp`)
...
By default, the app scans `results/` and `data/` continuously and refreshes periodically.
You can override scan roots with `MSH_SCAN_DIRS` (comma-separated paths).
