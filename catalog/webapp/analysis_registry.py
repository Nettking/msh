"""Lightweight analysis registry for the Streamlit digital twin webapp."""
...
def _read_preview_frame(path: Path, max_rows: int = 3000) -> pd.DataFrame:
...
def _build_artifact(path: Path, source_dir: str) -> DataArtifact:
    ...
    frame = pd.DataFrame()
    try:
        frame = _read_preview_frame(path)
...
def load_artifact_frame(path: str) -> pd.DataFrame:
    source = Path(path)
