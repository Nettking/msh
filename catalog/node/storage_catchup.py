"""Run one verified F4.1 former-primary catch-up over the relay."""
...
async def _connect(arguments: argparse.Namespace) -> RelayNodeClient:
    ...
    if enrollment == EnrollmentState.UNENROLLED.value and not token:
        raise FederationValidationError(
            "storage-catchup-enrollment-required",
            DEFAULT_ENROLLMENT_TOKEN_ENV,
    ...
    if arguments.session_id not in joined:
        ...
        if not invitation:
            await client.disconnect(error_code="storage-catchup-session-required")
            raise FederationValidationError(
                "storage-catchup-session-invitation-required",
                DEFAULT_SESSION_INVITATION_ENV,
        ...
        if session.get("session_id") != arguments.session_id:
            await client.disconnect(error_code="storage-catchup-session-mismatch")
            raise FederationValidationError(
                "storage-catchup-session-mismatch",
                "session_id",
    ...
    try:
        ...
        catchup = LiveFormerPrimaryCatchupCoordinator(
            ...
            catchup_store=LiveCatchupStore(
                Path(arguments.catchup_database)
            ),
            session_id=arguments.session_id,
        )
        result = await catchup.run_once(
            group_id=arguments.group_id,
            returning_provider_id=arguments.returning_provider_id,
            limit=arguments.limit,
        )
        print(json.dumps(result.to_dict(), ensure_ascii=False, sort_keys=True))
        return 0 if result.status in {"caught-up", "retryable"} else 2
    finally:
...
def main() -> int:
    ...
    try:
        ...
        if isinstance(exc, FederationValidationError):
            ...
            error = {
                "code": "storage-catchup-runtime-error",
                "field": "runtime",
