...
def lifecycle_worker(
    tmp_path: Path, provider_id: str, revision: int
) -> CancellableCapabilityWorker:
    return CancellableCapabilityWorker(
        ...
        SyntheticHandler(),
        SQLiteLifecycleDispatchInbox(
            tmp_path / f"lifecycle-{provider_id}-{revision}.sqlite3"
        ),
        clock=lambda: NOW,
    )
...
def test_f86_lifecycle_worker_replacement_rolls_back_exactly(tmp_path: Path) -> None:
    unrelated = lifecycle_worker(tmp_path, "provider-unrelated", 1)
    previous = lifecycle_worker(tmp_path, "provider-reconciled", 1)
    replacement = lifecycle_worker(tmp_path, "provider-reconciled", 2)
