...
def test_legacy_role_names_are_only_command_profile_aliases() -> None:
    ...
    assert all("FCP_DEPLOYMENT_MODE" not in line for line in env_lines_for_plan(plan))
...
def test_provider_profile_is_local_and_uses_only_real_compose_profile() -> None:
    ...
    assert "FCP_PROVIDER_MODEL=smollm2:360m" in env_lines_for_plan(plan)
...
def test_old_mode_flag_is_retained_as_cli_alias(monkeypatch) -> None:
    monkeypatch.setattr(
        sys,
        "argv",
        ["setup_fcp.py", "--mode", "web-workbench", "--no-ai"],
    )

    args = setup_fcp.parse_args()
...
def test_fresh_command_setup_writes_capability_config_not_legacy_role(
    ...
    assert "FCP_DEPLOYMENT_MODE" not in env_text
    assert "FCP_SKIP_ORCHESTRATION=0" in env_text
...
def test_one_shot_profile_does_not_persist_product_configuration(
    ...
    monkeypatch.setattr(setup_fcp, "_run_compose", lambda *_args, **_kwargs: None)
