...
def plan_contribution_publications(
    *,
    ...
    intent_by_id = {
        str(getattr(intent, "candidate_id")): intent
    ...
    for candidate_id, existing in existing_by_id.items():
        if candidate_id in current_ids:
            continue
        properties = existing.get("properties")
        if not isinstance(properties, dict) or properties.get("kind") != _PUBLICATION_KIND:
            continue
