...
def _result_band(row: dict[str, Any]) -> str:
    return str(
        row.get("result", {}).get("assessment", {}).get("key") or "unavailable"
    )
...
def _recommend(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    acceptable = [
        row for row in successful if _result_band(row) in ACCEPTABLE_BANDS
    ]
    ...
    if acceptable:
        recommended = max(
            acceptable,
            key=lambda row: PROFILE_STRENGTH[str(row["profile"])],
        )
        ...
        return {
            ...
            "message": (
                f"Recommended model: {recommended['label']} ({recommended['model']}). "
                f"It responded in {elapsed} ms, which is {assessment.lower()}."
            ),
        }
    ...
    if marginal:
        ...
        return {
            ...
            "recommended_label": recommended["label"],
            "message": (
                "This computer is marginal for local AI. Use "
                f"{recommended['label']} ({recommended['model']}) only if a "
                f"{elapsed} ms wait is acceptable."
            ),
        }
    ...
    if edge and edge.get("tested") and _result_band(edge) == "too_slow":
        return {
            ...
            "message": (
                "This computer does not appear suitable for local FCP AI. Even "
                "Edge small (smollm2:360m) was too slow. On old Raspberry Pi-class "
                "hardware, use a stronger computer for the language-model capability."
            ),
        }
    ...
    if edge and edge.get("tested") and not edge.get("result", {}).get("ok"):
        return {
            ...
            "message": (
                "The smallest model failed during benchmarking. Check Ollama logs. "
                "If it timed out on old Raspberry Pi-class hardware, use a stronger "
                "computer for the language-model capability."
            ),
        }
    ...
    if not edge or not edge.get("installed"):
        ...
        return {
            ...
            "message": (
                "No installed model proved usable. Install and test Edge small "
                "(smollm2:360m) before deciding that this computer is unsuitable."
            ),
        }
    ...
    return {
        ...
        "message": (
            "The benchmark did not produce enough information to recommend a model."
        ),
    }


def compare_ollama_setup_models(config: CapabilityConfig) -> dict[str, Any]:
    """Benchmark installed standard models using technical capability config."""

    status = ollama_status(config, timeout_seconds=2.0)
    if not status.get("running"):
        ...
        return {
            ...
            "recommendation": {
                ...
                "recommended_label": choice["label"],
                "message": (
                    "Ollama is not reachable. Start Ollama and test Edge small first "
    ...
    for profile, choice in AI_MODEL_CHOICES.items():
        ...
        if not installed:
            ...
            continue
        result = benchmark_ollama_response_time(config, model=choice["model"])
