...
def _utc(value: Any, field: str = "issued_at") -> datetime:
...
class StorageControlPlan:
    ...
    def __post_init__(self) -> None:
        ...
        if not all(isinstance(item, dict) for item in self.providers):
            raise FederationValidationError(
                "invalid-storage-control-plan", "providers", "must contain objects"
            )
        if not all(isinstance(item, dict) for item in self.groups):
            raise FederationValidationError(
                "invalid-storage-control-plan", "groups", "must contain objects"
            )
        provider_ids = [
            _text(item.get("provider_id"), "provider_id") for item in self.providers
        ]
    ...
    def calculated_hash(self) -> str:
        return _content_hash(self.unsigned_dict())
    ...
    @classmethod
    def from_dict(cls, value: Any) -> StorageControlPlan:
...
def _signed_plan(
    *,
    ...
    return StorageControlPlan(
        ...
        providers=providers,
        groups=groups,
        content_hash=_content_hash(unsigned),
...
class StorageControlPublicationStore:
        ...
        groups = tuple(
            {
                ...
                "grant": self._grant(snapshot.leader_grants.get(group_id)),
            }
    ...
    @staticmethod
    def _grant(value: dict[str, Any] | None) -> dict[str, Any] | None:
        if value is None:
            return None
        return {
...
class StorageControlReplicaStore:
        ...
        *,
        ...
        if current is not None:
            ...
            if plan.publication_revision == current.publication_revision:
                ...
                return self._result("duplicate", plan, control)
        ...
        with self._connect() as connection:
            connection.execute("BEGIN IMMEDIATE")
            row = connection.execute(
                """SELECT publication_revision FROM storage_control_replica_publications
                   WHERE session_id=?""",
                (plan.session_id,),
            ).fetchone()
            if row is not None and int(row["publication_revision"]) > plan.publication_revision:
                connection.rollback()
                raise FederationValidationError(
            ...
            connection.commit()
        return self._result("applied", plan, control)
    ...
    def _reconcile(self, plan: StorageControlPlan, control: PhaseDControlPlane) -> None:
        ...
        desired_providers = self._providers(plan)
        desired_groups = {str(value["group_id"]): value for value in plan.groups}
        snapshot = control.snapshot(session_id)
        if set(snapshot.groups).difference(desired_groups) or set(
            snapshot.providers
        ).difference(desired_providers):
            raise FederationValidationError(
        ...
        for provider_id, desired in sorted(desired_providers.items()):
            if control.snapshot(session_id).providers.get(provider_id) != desired:
                control.register_provider(session_id, plan.authority_node_id, desired)
        for group_id, desired in sorted(desired_groups.items()):
            self._reconcile_group(plan, control, group_id, desired)

    @staticmethod
    def _providers(plan: StorageControlPlan) -> dict[str, StorageProviderRegistration]:
        result: dict[str, StorageProviderRegistration] = {}
        for value in plan.providers:
            ...
            if provider.session_id != plan.session_id:
                raise FederationValidationError(
                    "storage-control-session-mismatch",
                    "provider.session_id",
                    "provider belongs to another session",
                )
    ...
    def _reconcile_group(
        ...
        assignment = control.snapshot(plan.session_id).groups[group_id]
        if (
        ...
        if control.acknowledgement_policy(plan.session_id, group_id).mode is not mode:
            control.set_acknowledgement_policy(plan.session_id, group_id, mode)
    ...
    def _reconcile_grant(
        ...
        expiry = _utc(desired.get("lease_expires_at"), "grant.lease_expires_at")
        ...
        desired_binding = (provider_id, grant_id, term, token, expiry, scopes)
        if current is not None:
            current_binding = (
                ...
                _utc(current.get("lease_expires_at"), "grant.lease_expires_at"),
                tuple(current.get("scopes", ())),
            )
            if current_binding == desired_binding:
                return
            if term <= int(current["term"]) or token <= int(current["fencing_token"]):
                raise FederationValidationError(
...
class StorageControlRelayPublisher:
    ...
    async def publish(
        ...
        targets = tuple(
            dict.fromkeys(_text(item, "target_node_id") for item in target_node_ids)
        )
        if not targets:
            raise FederationValidationError(
        ...
        while pending:
            ...
            if (
                ...
                or getattr(message, "session_id", None) != plan.session_id
